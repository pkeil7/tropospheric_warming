...
def low_freq_filter(ds,i1,i2,j1,j2) :
    fft = calculate_2dft(ds)
    fft_back_low = np.zeros_like(fft)
    fft_back_low[i1:i2,j1:j2] = fft[i1:i2,j1:j2] # the central indices of each dimension show
    # the lowest frequency ad are strongest on the power spectrum. That's why we use these indices
    return calculate_2dift(fft_back_low)

def xr_low_freq_filter(ds,kwargs) :
    return xr.apply_ufunc(
    low_freq_filter, ds,
    kwargs = kwargs,
...
def pressure2height(pressure,T0=288.15,gamma = -6.5e-3,P0 = 101325):
    ...
    For the tropopause region and the lower stratosphere (250-50hPa) we use a constant temperature of 216.65 K (see also here https://de.wikipedia.org/wiki/Normatmosph%C3%A4re)
    '''
...
def xr_fldmean(ds):
    ''' zonalmean that uses xarrays weighted function
    ...
    for lon in ['lon', 'longitude']:
        if lon in ds.coords:
            lon_name = lon
            return ds_weighted.mean([lat_name,lon_name])
        else :
            return ds_weighted.mean(lat_name)
...
def good_colorbar(im,fig,label,margins=[0.81, 0.15, 0.02, 0.7],label_kwargs={"labelpad": 20},cb_kwargs={}) :
    fig.subplots_adjust(right=0.8)
    cbar_ax = fig.add_axes(margins)
    cbar = fig.colorbar(im, cax=cbar_ax,**cb_kwargs)
    cbar.set_label(label, rotation=270,**label_kwargs)
    return cbar


# custom colormap with white in the middle:

colors1 = plt.cm.PiYG(np.linspace(0, 0.49, 100))
colors2 = plt.cm.PiYG(np.linspace(0.51, 1, 100))
colors_white = plt.cm.PiYG(np.linspace(0.49, 0.51, 56))
colors = np.vstack((colors1,colors_white, colors2))
myPiYG = mcolors.LinearSegmentedColormap.from_list('my_colormap', colors)

# a colourblind friendly collection of colours.
# With thanks to my friend Chris Sauter
...
collist = [(0,0,0),  # black
               ...
               (204/255, 121/255, 167/255),   # Redish purple
                ]
...
def distance(lat_a,lon_a,lat_b,lon_b):
    ...
    lat1 = np.deg2rad(lat_a)
    lon1 = np.deg2rad(lon_a)
    lat2 = np.deg2rad(lat_b)
    lon2 = np.deg2rad(lon_b)
...
def distance_lon(lon_a,lon_b,lats):
    R = 6373.0
    ...
    a = np.cos(lat)**2 * np.sin(dlon / 2)**2
    c = 2 * np.arctan2(np.sqrt(a), np.sqrt(1 - a))
...
def y_derivative(ds) :
    lats = ds.isel(lat=slice(1,-1)).lat
    deriv = xr.zeros_like(ds.isel(lat=slice(1,-1)))

    for i,lat in enumerate(lats) :
        lat1 = float(ds.isel(lat=i-1).lat.values)
        lat2 = float(ds.isel(lat=i+1).lat.values)
        dist = distance_lat(lat1,lat2) * 1000.
        deriv.loc[dict(lat=lat)] =  (ds.isel(lat=i+1) - ds.isel(lat=i-1)) / dist

    return deriv

def x_derivative(ds,cyclic=True) :
    deriv = xr.zeros_like(ds)
    lats = ds.lat.values
    lons = ds.isel(lon=slice(1,-1)).lon # to do: implement cyclical bc

    for i,lon in enumerate(lons) :
        lon1 = float(ds.isel(lon=i-1).lon.values)
        lon2 = float(ds.isel(lon=i+1).lon.values)
        dist = distance_lon(lon1,lon2,lats) * 1000.
        dist_da = xr.DataArray(dist, coords=[lats], dims=["lat"])
        deriv.loc[dict(lon=lon)] =  (ds.isel(lon=i+1) - ds.isel(lon=i-1)) / dist_da

    # cyclical boundary conditions

    if cyclic :
        lon_first = ds.isel(lon=0).lon.values
        lon_second = ds.isel(lon=1).lon.values
        lon_last = ds.isel(lon=-1).lon.values
        lon_second_last = ds.isel(lon=-2).lon.values

        dist = distance_lon(lon_last +360.,lon_second,lats) * 1000.
        deriv.loc[dict(lon=lon_first)] = (ds.isel(lon=1) - ds.isel(lon=-1)) / dist
        dist = distance_lon(lon_second_last +360.,lon_first,lats) * 1000.
        deriv.loc[dict(lon=lon_last)] = (ds.isel(lon=0) - ds.isel(lon=-2)) / dist
...
def xr_potential_temperature(ds):
    T = ds.ta
...
def static_stability(T, theta, dthetadp) :

    return - (T/theta) * dthetadp

def xr_static_stability(dataset, T='ta', p='plev') :
    """This function uses xarray datasets of temperature and pressure to calculate static stability.
    The temperature dataset should have vertical pressure coordinates in hPa.
    Potential Temperature is caluclated with the potential_temp function.
    This is then derived by pressure in hPa with the xarray function differentiate().
    The result is a dataarray"""

    theta = xr.apply_ufunc(
        potential_temp, dataset[T], dataset[p],
        dask='parallelized',
        output_dtypes=[float])

    dtheta_dp = theta.differentiate(p)

    sigma = xr.apply_ufunc(
        static_stability, dataset[T], theta, dtheta_dp,
        dask='parallelized',
        output_dtypes=[float])

    return sigma.to_dataset(name='sigma')



def xr_static_stability_hPa(dataset, T='ta', p='plev2') :
    """This function uses xarray datasets of temperature and pressure to calculate static stability.
    The temperature dataset should have vertical pressure coordinates in hPa.
    The result is a data array with static stability in K/hPa
    """

    theta = xr.apply_ufunc(
        potential_temp, dataset[T], dataset[p]*100,
        dask='parallelized',
        output_dtypes=[float])
...
def xr_geopotential(T,start_index=0):

    assert isinstance(T,xr.DataArray), " Object must be xr DataArray!"

    # check whether pressure coordinate is descending:
    T = T.sortby("plev", ascending=False)
    p_start = T.isel(plev2=start_index).plev2.values
    T_out = []

    for i,p in enumerate(T.plev2) :

        T_current = T.sel(plev2=slice(p_start,p))
        # determine integrand. For low pressure over alnd, sometimes temperature is NaN.
        # these Nans are replace with zero so they don't contribute to integration.
        integrand = - (T_current.fillna(0) * mt.Rd) / T_current.plev
        T_out.append(integrand.integrate("plev"))

    return xr.concat(T_out, T.plev2).to_dataset(name="Phi")

def xr_thickness(T,lower_level,upper_level):

    # check whether pressure coordinate is descending:
    T = T.sortby("plev", ascending=False)
    levels = T.sel(plev2=slice(lower_level,upper_level)).plev2
    assert len(levels) > 2, "not enoough levels! specifiy range so that you have a t least 2 levels!"

    T_ = T.sel(plev2=slice(lower_level,upper_level))
    integrand = - (T_ * mt.Rd) / T_.plev
...
# We start with a differential form, dX that allows us to construct how temperature changes with pressure following different processes.
# The form we construct is similar to $\theta_\mathrm{l}$ allows for a saturated isentrope allowing only for a liquid phase, a saturated isentrope assuming only a solidphase,
# whereby the saturated vapor pressure over ice is set to that over water at temperatures above 273.15 K.  We also calculate the pseudo adiabats, one allowing only a liquid phase, and another that transitions to the ice-phase at temperatures below 273.15 K,
#and differs from the former by the additional fusion enthalpy.  The true reversible isentrope would follow the saturated liquid phase isentrope at temperatures above the triple point and the saturated ice-phase isentrope at temperatures below the triple point.

# Equations are absed on Stevens and Siebesma Chapter 2, section 2.2.2, page 13

# Here we calcualte dX_dT and dX_dP. X is not really anything physically usefull, but is used in usefull maths trick here. dX = dh + vdp = 0, dX is always zero in an adiabatic process. we want to find a way to express dX in terms of dT and dp,
# so that dX = dX_dT dT + dX_dP dP. That gives us expressions for dX_dT, dX_dP. This is done with dh = cp dT + qs lv. qs and lv then again depend on p and T. The formula gets complicated, but we can change the terms around to get the
# dX = dX_dT dT + dX_dP dP = 0 structure and thereby find an expression for dX_dT, dX_dP. Then we just say: dT_dP = dX_dP / dX_dT. To get T(p) we integrate dX_dP / dX_dT by p.
